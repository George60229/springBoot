...
import com.epam.esm.mapper.GiftMapper;
...
public class CertificateDAOImpl extends EntityDAOImpl implements CertificateDAO {
    ...
    public List<GiftCertificate> listItems(CertificateFindByDTO certificateFindByDTO) {
        ...
        if (certificateFindByDTO.getFindParameter().name().equals("DESCRIPTION")) {
            ...
            sqlQueue = FIND_ALL_QUEUE + " where certificate_info.name like '%" + certificateFindByDTO.getValue() + "%'";
        } else {
    ...
    public boolean deleteById(Integer id) {
        String firstSql = "delete from certificates_to_tages where certificate_id=" + id;
        jdbcTemplateObject.update(firstSql);
        String secondSql = "delete from certificates where certificate_id=" + id;
        return jdbcTemplateObject.update(secondSql) > 0;
    }
